...
pub fn main() {
    ...
    let usage = "blockworld (resource_folder) (username) (port) <max_nodes>";
    ...
    let max_nodes = args.next().unwrap_or("300000000".into());
    let max_nodes: u32 = max_nodes
        .parse()
        .with_context(|| "Invalid max_nodes value")
        .expect(&format!(
            "Invalid cmdline arg \"max_nodes\"\nUsage: {usage}"
        ));

    let mut app_state = AppState::new(username, res_folder, port, max_nodes);
